...
- 📱 **Gerador de QR Code Inteligente**:
  - Geração automática do QR Code com o link direto do quiz (`/?quiz=CODIGO`).
...
- 📊 **Dashboard Dinâmico de Resultados**:
  ...
  - **Tabela de Classificação/Ranking**: Com pódio (🥇 🥈 🥉) e exportação dos dados para **CSV (Excel)**.
...
├── .streamlit/
│   └── config.toml          # Configurações de tema escuro e servidor
├── modules/
...
No painel do professor, o sistema já detecta o IP da sua rede local para gerar o QR Code. Basta projetar a tela e os alunos conectados ao mesmo Wi-Fi poderão escanear e responder!
...
4. Clique em **"Deploy!"**.
5. Em poucos segundos, sua aplicação terá um link público seguro (ex: `https://seu-quiz.streamlit.app`).
6. Cole essa URL no campo **"Endereço Base"** na Área do Professor para gerar os QR Codes com o link público!
